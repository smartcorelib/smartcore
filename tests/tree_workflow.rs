...
fn decision_tree_classifier_inline_workflow() {
    ...
    let model = DecisionTreeClassifier::fit(&x, &y, params)
        .expect("DecisionTreeClassifier::fit");
    let preds = model.predict(&x).expect("predict");

    let acc = accuracy(&preds, &y);
    assert!(acc >= 0.875, "DecisionTreeClassifier XOR accuracy: {acc:.3}");
}
...
fn decision_tree_regressor_inline_workflow() {
    ...
    let model = DecisionTreeRegressor::fit(&x, &y, params)
        .expect("DecisionTreeRegressor::fit");
    let preds = model.predict(&x).expect("predict");
...
fn decision_tree_classifier_iris_workflow() {
    ...
    assert!(acc >= 0.90, "DecisionTreeClassifier (iris) accuracy: {acc:.3}");
}
